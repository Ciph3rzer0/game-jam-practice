...
func _ready() -> void:
	# This script may be attached directly to the controlled CharacterBody3D.
	# In that case there is no exported actor assignment, so use this node.
	if actor == null:
		actor = get_node(".") as CharacterBody3D

	if actor == null:
		push_error("PlayerController needs an actor or must be attached to a CharacterBody3D.")
...
func _get_camera_relative_movement_vector(input_vector: Vector2) -> Vector3:
	...
	var camera_forward := -current_camera.global_basis.z
	camera_forward.y = 0.0
	var camera_right := current_camera.global_basis.x
	camera_right.y = 0.0
